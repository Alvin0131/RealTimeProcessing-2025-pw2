...
extern "C" void gaussianBlurCUDA(unsigned char* inputPtr, unsigned char* outputPtr, int width, int height, double* kernel, int kernelSize);
extern "C" void adaptiveDenoiseCUDA(unsigned char* inputPtr, unsigned char* outputPtr, int width, int height, int neighborhoodSize, double factorRatio);
...
int main() {
    ...
    if (choice == "denoise") {
        ...
        Mat leftBlurred = leftTransformed.clone();
        Mat rightBlurred = rightTransformed.clone();
        adaptiveDenoiseCUDA(leftTransformed.ptr<unsigned char>(), leftBlurred.ptr<unsigned char>(), leftTransformed.cols, leftTransformed.rows, 3, 20.0);
        adaptiveDenoiseCUDA(rightTransformed.ptr<unsigned char>(), rightBlurred.ptr<unsigned char>(), rightTransformed.cols, rightTransformed.rows, 3, 20.0);
        ...
        Mat leftBlurred = leftTransformed.clone();
        Mat rightBlurred = rightTransformed.clone();
        gaussianBlurCUDA(leftTransformed.ptr<unsigned char>(), leftBlurred.ptr<unsigned char>(), leftTransformed.cols, leftTransformed.rows, kernel.ptr<double>(), kernel.rows);
        gaussianBlurCUDA(rightTransformed.ptr<unsigned char>(), rightBlurred.ptr<unsigned char>(), rightTransformed.cols, rightTransformed.rows, kernel.ptr<double>(), kernel.rows);
